...
class Player():
    ...
    def suggestACard(self, topCardInPile, attemptsRemaining):
        ...
        This method should remain abstract within Player; only subclasses
        should implement suggestACard.
...
class RankThenSuitAiPlayer(Player):
    ...
    def suggestACard(self, topCardInPile, attemptsRemaining):
        ...
        """
        # TODO: Students, implement the suggestACard method here!
        # This is a special edge case where the AI is the first player.
        if topCardInPile == None:
            return self._pickRandomCardHelper()
        
        # Find the first non-wild card matching the rank.
        i = 0
        while i < len(self.hand):
            card = self.hand[i]
            i += 1
            if card.rank == topCardInPile.rank and not isinstance(card, WildUnoCard):
                return card
        
        # If we got here, find the first non-wild card matching the suit.
        i = 0
        while i < len(self.hand):
            card = self.hand[i]
            i += 1
            if card.suit == topCardInPile.suit and not isinstance(card, WildUnoCard):
                return card
        
        # We don't have a matching card, so we probably need to use a wild card if we have one.
        i = 0
        while i < len(self.hand):
            card = self.hand[i]
            i += 1
        ...
        # Oh no, we couldn't find a card smartly; just return one randomly and hope it works out.
        return self._pickRandomCardHelper()
